# 004 要求定義 — Data Analyst Agent

## 6.1 Meta
- pipeline_stage: request_definition
- version: 1.0.0
- agent_id: pawmate_a18
- source: `pawmate_agent_team.md` / `A18｜Data Analyst Agent`
- priority_band: Phase 3 candidate

## 6.2 Title
Data Analyst Agent に関する要求定義（PawMate / AFOS 003-010）

## 6.3 Background & Problem
PawMateは「広げる前に深める。深まったら広げる。」を基本方針とし、まず茅ヶ崎で最初の取引を成立させる。Data Analyst Agent は、リピート率、GMV、LTV、稼働率、Sitter-to-Owner比率の分析。
...
- 初回プロンプト例: PawMateのData Analyst Agentとして、担当チケットを前に進めるための論点、成果物、Keiの確認事項を整理してください。
...
| 要求項 | 根拠 |
|---|---|
| 役割 | `pawmate_agent_team.md` A18 |
| チケット | `pawmate_tickets.md` / 本エージェントの見るべきチケット |
| Gate | `pawmate_roadmap.md` / `pawmate_daily_calendar_fast.md` |
| 成果物 | 本エージェントの標準成果物 |

## 6.12 Review Summary
005では本要求をREQへ分解し、006でSEC構造へ配賦する。007以降では、PawMate共通ルールとData Analyst Agent固有の出力形式を固定する。
