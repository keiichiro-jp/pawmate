# 005 要件定義 — Vibe Engineering Agent
...
### REQ-FR-010 プロトタイプ改修（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。

### REQ-FR-011 データ構造更新（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。

### REQ-FR-012 UI実装（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。

### REQ-FR-013 Stripe Connect（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。

### REQ-FR-014 報告カード（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。

### REQ-FR-015 Meet & Greet（P1）
- 担当領域として必要な場合、具体的な判断材料またはドラフトへ落とす。
...
- ユーザー承認なしの本番デプロイ、破壊的変更、秘密情報の露出 は行わない。
